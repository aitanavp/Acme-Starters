...
<jstl:choose>
        ...
        <acme:form> 
            ...
            <jstl:choose>    
                <jstl:when test="${draftMode == true}">
                    <acme:button code="project-member.project.form.button.members" action="/manager/project/members?projectId=${id}"/>
    ...
    <jstl:when test="${_command == 'members'}">
        <acme:list>
            <acme:list-column code="project-member.project.form.members.members" path="projectMember" width="80%"/>
        </acme:list>
        <jstl:if test="${draftMode == true && canManageMembers == true}">
            <acme:button code="project-member.project.form.button.add-members" action="/manager/project/add-members?projectId=${projectId}"/>
            <acme:button code="project-member.project.form.button.delete-members" action="/manager/project/delete-members?projectId=${projectId}"/>
        </jstl:if>
    </jstl:when>
    ...
    <jstl:when test="${_command == 'add-members'}">
        <acme:form>
            ...
            <acme:submit code="project-member.membership.form.button.add" action="/manager/project/add-members?projectId=${projectId}"/>
        </acme:form>
    </jstl:when>
    ...
    <jstl:when test="${_command == 'delete-members'}">
        <acme:form>
            <acme:form-select code="project-member.membership.form.projectMember" path="projectMember" choices="${projectMemberChoices}"/>
            <acme:submit code="project-member.membership.form.button.delete" action="/manager/project/delete-members?projectId=${projectId}"/>
        </acme:form>
    </jstl:when>
</jstl:choose>
